...
#### set up ####
##library(tidyverse) ; library(lubridate) ; library(janitor) ; library(hms) ; library(readxl) ; library(data.table) ; library(spatsoc)
...
ate <- readxl::read_xlsx('../data_raw/Raw_ATE_MaleSightingsCleaned_Fishlock220808.xlsx') %>% 
  janitor::clean_names()
locations <- readxl::read_xlsx('../data_raw/Raw_ATE_Sightings_Fishlock220224.xlsx') %>% janitor::clean_names()
...
ate <- ate %>% 
  left_join(distinct(locations[,c('obs_id','casename','obs_num','utm_lat','utm_long','bull_q_r','obs_type')]),
            by = c('obs_id','casename')) %>% 
  select(-bull_id) %>% 
  relocate(obs_id) %>% relocate(obs_num, .after = obs_time) %>% relocate(obs_type, .after = obs_num) %>% 
  relocate(utm_lat, .after = grid_code) %>% relocate(utm_long, .after = utm_lat) %>% 
  relocate(bull_q_r, .after = 'hab_code_2')

colnames(ate)[c(6,7,9,10,13)] <- c("obs_num_old","obs_type_old","utm_lat_old","utm_long_old","bull_q_r_old")
